...
var tools = {
    ...
    renderSelect:{
        // 新增商品分类选择
        // categoryId: function(select){
        //     var categoryIdData = cacheSessionStorage.getCache('categoryIdData');
        ...
        //                     cacheSessionStorage.setCache('categoryIdData',result.body);
        //                     categoryIdData = result.body;
        //                 } else {
        ...
        //     }
        //     var optHtml = '';
        //     categoryIdData.map( (item, index) => {
        //         if(item.adminCategories.length > 0){
        //             item.adminCategories.map( (itemIn, indexIn) => {
        //                 if(itemIn.adminCategories.length > 0){
        //                     itemIn.adminCategories.map( (itemInIn, indexInIn) => {
        //                         optHtml += '<option value="'+ itemInIn.id+'">' + itemInIn.name + '</option>';
        //                     })
        //                 }else{
        //                     optHtml += '<option value="'+ itemIn.id+'">' + itemIn.name + '</option>';
        //                 }
        //             })
        //         }
        //     })
        ...
        // },
        // 新增聚合选择
        // aggregaTogether: function(select, scopeId){
        //     var aggregaTogetherData = cacheSessionStorage.getCache('aggregaTogetherData');
        //     if(!aggregaTogetherData || !aggregaTogetherData.length > 0){
        //         $.ajax({
        //             type: "GET",
        //             url: utils.prePath() + "/api/admin/aggregate/page/list"  + tools.jsonToParameString({sort: 'priority',size: 100,page: 0,scopeId: scopeId}),
        //             async: false,
        //             success: function (result) {
        //                 if (result.statusCode == 'OK') {
        //                     cacheSessionStorage.setCache('aggregaTogetherData',result.body);
        //                     aggregaTogetherData = result.body.content;
        //                 } else {
        //                     toastr.error((result.body && result.body.message) ? result.body.message : '系统异常,请稍候重试或联系开发人员...');
        //                 }
        //             }
        ...
        //     }
        //     var optHtml = '';
        //     aggregaTogetherData.map( (item, index) => {
        //         if(item.childrens.length > 0){
        //             item.childrens.map((itemIn, indexIn) => {
        //                 optHtml += '<option value="'+ itemIn.id+'">' + itemIn.name + '</option>';
        //             })
        //         }
        //     })
        ...
        // 新增价格设置版本
        priceSetting: function(select,scopeId){
            let allPrice = [],priceHtml='';
            $.ajax({
                type: "GET",
                url: utils.prePath() + "/api/admin/product/price/version?scopeId=" + scopeId,
                async: false,
                success: function(result){
                    if(result.statusCode == 'OK'){
                        allPrice = result.body;
                    }else{
                        toastr.error((result.body && result.body.message) ? result.body.message : '系统异常,请稍候重试或联系开发人员...');
                    }
                }
            ...
            if(allPrice.length > 0){
                allPrice.map((item, index) => {
                    priceHtml += '<option value="'+ item.id +'" data-time="'+ moment(item.updatedAt).format('YYYY-MM-DD HH:mm:ss') +'" data-name="' + item.updatedBy + '" data-id="'+ item.id +'">'+ item.version +'</option>'
                })
            }else{
                priceHtml = '<option value="" data-time="" data-id="" data-name="">暂无版本号</option>'
            }
            
            $(select).html(priceHtml).selectpicker('refresh'); 
        },
        category1Id: function(select, type){
            $.ajax({
                type: "GET",
                url: utils.prePath() + "/api/admin/erp/category/tree",
                async: false,
                success: function(result){
                    if(result.statusCode == 'OK'){
                        scopeData = result.body;
                        let cateHtml = '<option value="">全部分类</option>'
                        scopeData.map(function(e,i){
                            cateHtml += '<option value="'+e.id+'">' + e.name + '</option>';
                        })
                        $(select).html(cateHtml).selectpicker();
                    }else{
                        toastr.error((result.body && result.body.message) ? result.body.message : '系统异常,请稍候重试或联系开发人员...');
                    }
                }
        ...
        couponsList: function(select){
            var optHtml = `<option value="">所有折扣券</option><option value="50">无门槛</option><option value="10">金额满减券</option><option value="80">打折券</option>`
            $(select).html(optHtml).selectpicker()
        },
        ...
        storehouseId: function (select) {
            ...
            {value:'台湾仓',num:'9'},
            {value:'深圳仓（备选）',num:'10'},
            {value:'东莞仓',num:'11'}].map(function(e,i){
                optHtml += '<option value="'+e.num+'">' + e.value + '</option>';
            })
            $(select).html(optHtml).selectpicker();
        },
    ...
    formatterFunction:{
        ...
        returnCostStatus: function(value){
            var mes = '--',
                stateColor = '';
            switch (value) {
        ...
        deliveryStatus: function(value){
            ...
            switch(value - 0){
                ...
                case 50 : stateVal =  "已退回";stateColor = 'label-success';break;
            }
        ...
        serviceInfos: function(skuData,i){
            ...
            skuData.map(function(e,j){
                ...
                html += '<li>'+
                    ...
                    '<span>' + (e.returnQuantity|| "") + '</span>'+
                    '<span>' + (e.serviceType || "") + '</span>'+
                    '<span>' + (e.subServiceType || '') + '</span>'+
        ...
        skuInfos: function(value, no_pass_reson){
            ...
            skuData.map(function(e){
                ...
                html += '<li style="' + highlight + '">'+
                    ...
                    '<span>' +
                        (e.order_product_status > 0 && e.order_product_status <= 30 ? '<button authority="ORDER_WRITE" type="button" class="btn btn-danger btn-xs cancelOrderSku" data-ordersku="' + e.order_product_id + '">取消SKU</button>' : e.cancelReason )+
                    '</span>'+
                '</li>';
            });
            html += '</ul>';
        ...
        stockSkuInfos: function(skuData, state, isDel, isClose){
            ...
            skuData.map(function(e,j){
                html += '<li>'
                if(isClose){
                    html += '<span style="width:100px;">'+ e.product_id +'</span>' 
                }
                if(state){
                ...
                html += '<span style="text-align:center;width:300px;"><span>' + e.product_sku_name + '</span> - ' + (e.product_title ? e.product_title.replace('(','（').replace(')','）') : '') + '</span>'+
                    ...
                    html += '<button type="button" class="btn btn-info btn-xs editOrderSku" style="float:inherit;" data-ordersku="' + e.purchased_product_id + '" data-quantity="' + e.purchased_quantity + '"  data-storaged_quantity="' + e.storaged_quantity + '"><i class="fa fa-edit"></i></button>'
                if(state){
        ...
        customerType:function(lastOrderCreatedAt,createOrderCount,direct_cancel,storeTypeBlacklist){
            if(lastOrderCreatedAt==null || createOrderCount==1){
                ...
                if(storeTypeBlacklist && storeTypeBlacklist == "true"){
                    customerTypeHtml += '<p style="padding: 1px 4px;margin-left: 5px; background: #0c1312; color: #fff; display: inline-block;  border-radius:4px; font-size: 12px;">店配黑名单</p>';
                }
    ...
    rangeTime: function(select, initType, justDay, endType){
        ...
        var timeJson = {
            ...
            lastFifty: moment().subtract(16, 'days').format('YYYY-MM-DD')
        }
    ...
    // 新增方法存储全局设置变量
    globalSetting: function (globalKey){
        let returnGlobal = ''
        $.ajax({
            type: "GET",
            url:utils.prePath() + "/api/system/config/info",
            async:false,
            data: {
                key: globalKey
            },
            success : function(result){
                if(result.statusCode == 'OK'){
                    returnGlobal = result.body.globalValue
                }else{
                    toastr.error((result.body && result.body.message) ? result.body.message : '系统异常,请稍候重试或联系开发人员...');
                    return ''
                }
